...
Here is a list of publications from the SCAMs@bristol group and Andrew.

{% for pub in site.data.publications offset:page.number limit:page.per_page %}
  <body>
  <b>
    {{ pub.title }}
  </b><br>
  <small>
    <i>{{ pub.journal }}</i>
    {% if pub.volume.size > 0 %}, <b>{{ pub.volume }}</b>{% endif %}
